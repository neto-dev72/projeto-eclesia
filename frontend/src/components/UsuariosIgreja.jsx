...
export default function GestaoIgrejas() {
  ...
  const handleNovaSedeSubmit = async (e) => {
    e.preventDefault();
    try {
      // Usa a rota unificada /igrejas
      const res = await api.post('/igrejas', novaSede);
      setSedes([...sedes, res.data.sede || res.data.filhal]);
      setNovaSede({ nome: '', endereco: '', telefone: '', email: '', usuarioNome: '', usuarioSenha: '', usuarioFuncao: 'admin' });
